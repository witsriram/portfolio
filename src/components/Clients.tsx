...
export default function Clients() {
  ...
  return (
    <section ref={ref} className={`relative overflow-hidden border-y py-6 ${
      isDark ? "border-[#111]" : "border-[#e0e0e0]"
    }`}>
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <motion.h2
          ...
          className={`mb-5 text-center text-xl font-semibold sm:text-2xl ${isDark ? "text-[#888]" : "text-[#666]"}`}
        >
          ...
          {clients.map((client, i) => (
            <motion.div
              ...
              className={`group flex items-center gap-2.5 rounded-xl border px-4 py-3 transition-all duration-300 ${
                isDark
                  ? "border-[#1a1a1a] bg-[#0a0a0a]/60 hover:border-[#2a2a2a]"
            ...
            >
              ...
              <span className={`text-sm font-semibold tracking-tight transition-colors duration-300 ${
                isDark ? "text-[#666] group-hover:text-white" : "text-[#999] group-hover:text-black"
              }`}>
                {client.name}
              </span>
              ...
              <span className={`rounded-md px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wider ${
                isDark ? "bg-[#111] text-[#444]" : "bg-[#f0f0f0] text-[#bbb]"
              }`}>
                {client.sector}
              </span>
            </motion.div>
